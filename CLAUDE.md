...
- Текущая разметка тем в `data/posts.json` — ручная (8 тем, размечено Клодом по содержанию).
  ...
  Проверенный источник — `data/reviewed-topics.json`. После импорта прочитай новые/изменённые
  посты, обнови assignments и запусти `npm run prepare-data -- --reviewed` для пересчёта карты
  с прежними темами. Подробный обязательный тюнинг и проверки описаны в README, раздел
